...
import { getMissingParams, getUpdatedFields } from "../../shared/utils/dynamicData";
...
export const upsertPermissions = asyncHandler(
  async (req: ExtendedRequest, res: Response): Promise<any> => {
    ...
    try {
      ...
      for (const perm of permissions) {
        ...
        if (existing) {
          const updatedFields = getUpdatedFields(existing, {
            scope: perm.scope,
          });
          if (Object.keys(updatedFields).length > 0) {
