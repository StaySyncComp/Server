...
import { ExtendedRequestGuest } from "../../shared/types/users/usersRequests";
import { prismaClient } from "../../shared/prisma";
import { emitToAuthorizedSockets } from "../../shared/utils/controllerUtils";
import { notifyMultipleUsers } from "../../shared/utils/expo";
import asyncHandler from "express-async-handler";
export const getGuestInformation = asyncHandler(
...
export const getOrganizationInformation = asyncHandler(
  async (req: ExtendedRequestGuest, res: Response): Promise<any> => {
    if (!req.user) return res.status(401).json({ message: "Unauthorized" });
...
export const listCallGuestCategoriesForAi = asyncHandler(
  async (req: ExtendedRequestGuest, res: Response): Promise<any> => {
    if (!req.user) return res.status(401).json({ message: "Unauthorized" });
    const { organizationId } = req.user;
    if (!organizationId)
...
export const createGuestCall = asyncHandler(
  async (req: ExtendedRequestGuest, res: Response): Promise<any> => {
    try {
