...
export const getAverageCloseTime = async (
  req: ExtendedRequest,
  res: Response
): Promise<void> => {
...
export const getTopClosers = async (
  req: ExtendedRequest,
  res: Response
): Promise<void> => {
  const { organizationId } = req.query;
...
export const getCallsByCategory = async (
  req: ExtendedRequest,
  res: Response
): Promise<void> => {
  const { organizationId } = req.query;
...
export const getStatusPie = async (
  req: ExtendedRequest,
  res: Response
): Promise<void> => {
  const { organizationId } = req.query;
  ...
  try {
    ...
    const peakHours = Object.entries(hourlyData).map(
      ([timeRange, callCount]) => ({
        timeRange,
        callCount,
        percentage: maxCalls > 0 ? Math.round((callCount / maxCalls) * 100) : 0,
      })
    );
